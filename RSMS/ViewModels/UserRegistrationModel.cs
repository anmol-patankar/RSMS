...
namespace RSMS.ViewModels
{
    public class UserRegistrationModel : IValidatableObject
    {
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            var oldDateCheck = Dob.AddYears(122);
            var youngDateCheck = Dob.AddYears(18);
            ...
            if (oldDateCheck.CompareTo(dateNow) < 0) results.Add(new ValidationResult("You are not that old, input a valid date", new[] { nameof(Dob) }));
            else if (youngDateCheck.CompareTo(dateNow) > -1) results.Add(new ValidationResult("You must be atleast 18 years old to register", new[] { nameof(Dob) }));
